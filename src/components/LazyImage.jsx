...
// Using sessionStorage so the cache persists across page reloads in the same tab
let loadedImagesCache;
try {
  const cached = sessionStorage.getItem('loadedImagesCache');
  loadedImagesCache = cached ? new Set(JSON.parse(cached)) : new Set();
} catch (e) {
...
const addImageToCache = (src) => {
  ...
  try {
    sessionStorage.setItem('loadedImagesCache', JSON.stringify(Array.from(loadedImagesCache)));
  } catch (e) {
